...
- **RC retain/release last-use elision (item, kept off).** A pass to cancel provably-redundant, non-escaping retain/release pairs is implemented and unit-tested but not wired into the body pipeline: its conservative escape gate never fires on the real corpus, so it carries the compiled-tier double-free risk without the benefit. It re-enables once gated on the true `SHARED_BIT` / goroutine boundary.
