...
- **API migrations are behavior-preserving.** `quick-xml` 0.40 now reports entity references as their own `GeneralRef` events; the XML codec reassembles and entity-resolves character data per element so output stays byte-identical across the VM and compiled tiers. `toml` 0.9 narrowed `Value`'s `FromStr` to value-expressions, so `is_valid` now parses a full document. No language-surface changes.
