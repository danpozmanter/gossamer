...
This is the Gossamer 0.42.2 standard library reference.
...
Receiver methods on built-in types such as `String`, `Vec`, `HashMap`, `Option`, and `Result` are listed in [Methods by type](method_support.md).
...
| [`std::fs`](#stdfs) | 25 | Filesystem reading, writing, and traversal (Rust std::fs shape). |
...
| [`std::iter`](#stditer) | 50 | Sequence adapters over Vec<T>: map, filter, fold, zip, enumerate, chain, etc. |
...
| [`std::time`](#stdtime) | 18 | Wall-clock and monotonic time facilities. |
...
| `BTreeSet` | type | Ordered set with `insert`, `contains`, `remove`, `len`, `is_empty`, `clear`, `iter`, `to_vec`, and set-algebra methods. Set literals use `#{a, b}` and empty set literals use `#{}` when an expected `BTreeSet<T>` type is present. |
| `BinaryHeap` | type | Compatibility alias for `MaxHeap<i64>`; heap literals use `^[a, b]` and empty heap literals use `^[]`. |
| `HashMap` | type | Hash map backed by the swiss-table layout. Map literals use `{key: value}` and empty map literals use `{}`. |
| `HashSet` | type | Unordered set with `insert`, `contains`, `remove`, `len`, `is_empty`, `clear`, `iter`, `to_vec`, and set-algebra methods. Set literals use `#{a, b}` and empty set literals use `#{}`. Like Rust's `HashSet`, mapping is an iterator operation: use `set.iter().map(f)`, not `set.map(f)`. |
| `MaxHeap` | type | Max heap. Phase 1 runtime support is `MaxHeap<i64>`; heap literals use `^[a, b]` and empty heap literals use `^[]`. |
| `MinHeap` | type | Min heap. Phase 1 runtime support is `MinHeap<i64>`; heap literals use `_[a, b]` and empty heap literals use `_[]`. |
| `Vec` | type | Growable contiguous sequence. Vec literals use `[a, b]` and empty Vec literals use `[]`. |
| `VecDeque` | type | Double-ended queue backed by a ring buffer. Phase 1 runtime support is `VecDeque<i64>`; queue literals use `<[a, b]>` and empty queue literals use `<[]>`. |
| `VecDequeue` | type | Alias for `VecDeque`; queue literals use `<[a, b]>` and empty queue literals use `<[]>`. |
| `VecQueue` | type | Alias for `VecDeque`; queue literals use `<[a, b]>` and empty queue literals use `<[]>`. |
...
| Item | Kind | Doc |
|------|------|-----|
| `server` | fn | server(status, body) -> String: starts an isolated loopback static-response server and returns its http:// base URL. Use http::get or http::Client as the test client. The server is test-process scoped and stops when that process exits. |
