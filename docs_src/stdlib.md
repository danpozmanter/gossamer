...
This is the Gossamer 0.55.2 standard library reference.
...
| [`std::fs`](#stdfs) | 30 | Filesystem reading, writing, and traversal (Rust std::fs shape). |
...
| [`std::lifecycle`](#stdlifecycle) | 7 | Process readiness and graceful shutdown, with systemd sd_notify. Shutdown is observed, not dispatched: wait for it, then drain with ordinary statements - `go serve()`, `lifecycle::ready()`, `lifecycle::await_shutdown()`, then the cleanup. |
...
| [`std::sandbox`](#stdsandbox) | 31 | Run a command under an OS-native sandbox: one policy model, three backends, no daemon or root. |
...
| [`std::sync`](#stdsync) | 14 | Synchronisation primitives beyond channels. |
...
| [`std::time`](#stdtime) | 23 | Wall-clock and monotonic time facilities. |
...
| `Display` | trait | How a value renders through `{}`. The rendering is synthesized; `impl Display for T { fn to_string(&self) -> String }` overrides it. |
...
| `clean_stale_grants` | fn | clean_stale_grants() -> Result<i64, errors::Error>. Revokes those grants and answers how many were revoked. |
| `exit_command_not_found` | fn | The exit code for a command that was not found inside the sandbox. |
| `exit_level_unavailable` | fn | The exit code for a level this host cannot honor. |
| `exit_policy_error` | fn | The exit code for a policy that would not compile, a sandbox that could not start, or a tree killed for exceeding its timeout. |
| `exit_signal_base` | fn | The number added to a signal when a child dies on one, so a wrapper reports what a shell would. |
| `expand` | fn | expand(text) -> Option<String>. A written path with ~ and environment references resolved, so a profile can name a path the way an operator writes it. |
| `filesystem` | fn | How completely a filesystem policy is enforced here: full, partial (reason), or none. |
| `filesystem_kind` | fn | full, partial, or none - the filesystem verdict as an arm to match on rather than a line to print. |
| `filesystem_reason` | fn | What a partial filesystem verdict does not cover, or the empty string when the verdict carries no reason. |
...
| `network_kind` | fn | full, partial, or none - the host's network verdict as an arm to match on. |
| `network_reason` | fn | What a partial network verdict does not cover, or the empty string. |
...
| `os_description` | fn | The host as the capability report names it: the OS and architecture a note about this machine would cite. |
| `platform` | fn | Which backend answers here: linux, macos, windows, or unsupported. |
| `prefix_of` | fn | prefix_of(name) -> Option<String>. The install prefix of a tool on PATH, for granting a toolchain without hard-coding where it lives. |
| `process_isolation` | fn | How completely the process table is isolated here. |
| `process_isolation_kind` | fn | full, partial, or none - the process-isolation verdict as an arm to match on. |
| `process_isolation_reason` | fn | What a partial process-isolation verdict does not cover, or the empty string. |
| `resolve_on_path` | fn | resolve_on_path(name) -> Option<String>. Where PATH resolves a command name. |
| `resource_limits` | fn | How completely resource limits are enforced here. |
| `resource_limits_kind` | fn | full, partial, or none - the resource-limit verdict as an arm to match on. |
| `resource_limits_reason` | fn | What a partial resource-limit verdict does not cover, or the empty string. |
...
| `run_inherit` | fn | run_inherit(policy, argv) -> i64. Runs argv with the caller's own streams and answers the exit code the contract gives, which is the shape a wrapper command wants. |
| `rust_toolchain_paths` | fn | Every path a Rust toolchain needs read access to, for a policy that has to run cargo. |
| `stale_grant_count` | fn | How many interrupted runs left an ACL grant behind. Always zero where granting a path is not how the backend reaches it. |
...
| `parse` | fn | Parses the string into the expected Result payload type. |
