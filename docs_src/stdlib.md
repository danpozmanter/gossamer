...
This is the Gossamer 0.44.0 standard library reference.
...
| [`std::fs`](#stdfs) | 26 | Filesystem reading, writing, and traversal (Rust std::fs shape). |
...
| [`std::path`](#stdpath) | 13 | Lexical filesystem-path operations; platform path grammar, no URL parsing. |
