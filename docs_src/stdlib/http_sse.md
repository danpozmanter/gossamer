...
|---|---|---|
| [`Event`](https://github.com/danpozmanter/gossamer/blob/main/crates/gossamer-std/src/http_sse.rs) | `type` — see the source declaration | One SSE event (id, event, data, retry). |
| [`Stream`](https://github.com/danpozmanter/gossamer/blob/main/crates/gossamer-std/src/http_sse.rs) | `type` — see the source declaration | Active SSE stream - handler writes events through it (Rust-side). |
