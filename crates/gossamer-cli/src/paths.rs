...
pub(crate) fn read_entry_source(file: &PathBuf) -> Result<String> {
    // A bare relative entry (`gos main.gos`) has an empty
...
pub(crate) fn bundle_sibling_modules(entry: &Path, source: String) -> String {
    ...
    // single-file `gos /tmp/foo.gos` invocations must NOT pick
...
/// loose-file invocations like `gos /tmp/foo.gos`.
...
/// This prevents `gos path/to/program_name` from trying to parse
