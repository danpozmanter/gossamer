#![allow(
    unused_imports,
    dead_code,
    unreachable_pub,
    missing_docs,
    clippy::wildcard_imports,
    clippy::too_many_lines,
    clippy::too_many_arguments,
    clippy::similar_names,
    clippy::many_single_char_names,
    clippy::items_after_statements,
    clippy::cast_lossless,
    clippy::cast_possible_truncation,
    clippy::cast_possible_wrap,
    clippy::cast_sign_loss,
    clippy::doc_markdown,
    clippy::option_if_let_else,
    clippy::match_same_arms,
    clippy::if_not_else,
    clippy::single_match_else,
    clippy::needless_pass_by_value,
    clippy::manual_let_else,
    clippy::redundant_else,
    clippy::collapsible_if,
    clippy::collapsible_else_if,
    clippy::map_unwrap_or,
    clippy::struct_excessive_bools,
    clippy::module_name_repetitions,
    clippy::unnecessary_wraps,
    clippy::large_enum_variant,
    clippy::if_same_then_else,
    clippy::single_match,
    clippy::useless_conversion,
    clippy::needless_borrows_for_generic_args,
    clippy::let_and_return,
    clippy::needless_collect,
    clippy::elidable_lifetime_names,
    clippy::missing_errors_doc,
    clippy::missing_panics_doc,
    clippy::must_use_candidate,
    clippy::missing_const_for_fn,
    clippy::needless_range_loop,
    clippy::cognitive_complexity,
    clippy::unused_io_amount,
    clippy::ptr_arg,
    clippy::ptr_as_ptr,
    clippy::redundant_closure,
    clippy::redundant_closure_for_method_calls,
    clippy::semicolon_if_nothing_returned,
    clippy::single_call_fn,
    clippy::unused_self,
    clippy::range_plus_one,
    clippy::missing_safety_doc,
    clippy::not_unsafe_ptr_arg_deref,
    clippy::cast_ptr_alignment,
    clippy::manual_assert,
    clippy::manual_string_new,
    clippy::match_bool,
    clippy::nonminimal_bool,
    clippy::redundant_pattern_matching,
    clippy::useless_let_if_seq
)]
//! Real Cranelift-backed native codegen.
//! Lowers a slice of MIR [`Body`]s into a `cranelift-object` module
//! and serialises the result as ELF (or the host's equivalent object
//! format). Supported today:
//! - `fn main() -> i64` with integer arithmetic (`+`, `-`, `*`, `/`,
//!   `%`, `&`, `|`, `^`, `<<`, `>>`, unary `-`, `!`),
//! - integer constants,
//! - direct calls between lowered functions,
//! - `return` of an `i64`.
//!
//! A C-ABI shim `main(argc, argv) -> i32` is emitted automatically:
//! it calls the Gossamer `main` and truncates the `i64` result into
//! the process exit code, so the object file links through a
//! standard `cc` invocation.
//! Aggregates (tuples/arrays/structs), strings, closures, and
//! anything that needs a GC heap are not yet lowered - those
//! constructs fall back to [`crate::emit::emit_module`] for
//! inspection.

// Allow patterns the Cranelift lowering deliberately uses:
//   - `similar_names` fires on `print_str`/`print_i64`/etc.
//     intrinsic-name shadowing within the same arm. The
//     parallel naming makes the dispatch table readable.
//   - `many_single_char_names` fires on hot inner-loop locals
//     (`a`, `b`, `n`, `m`, `k`) where longer names would
//     overflow the 100-col limit.
//   - `items_after_statements` flags inline `extern "C"` decls
//     localised to the one helper that uses them. Hoisting them
//     to module scope spreads the FFI surface; localised wins.
//   - `too_many_lines` / `cognitive_complexity` fire on the
//     intrinsic-dispatch arm and the `lower_intrinsic_call`
//     match. Splitting either hides the one-arm-per-symbol
//     structure that makes the table grep-able.
//   - `unnecessary_wraps` flags helpers whose `Result` exists
//     so call sites can still `?` them once a future lowering
//     can fail.
//   - `if_chain_can_be_rewritten_with_match` would flatten
//     short `if let Some(x) = .. else if let Some(y) = ..`
//     chains into match-on-tuple-of-options that's strictly
//     uglier here.
//   - `doc_markdown` flags identifiers like `i64`, `f64`,
//     etc. in plain-prose docs. Backticking every numeric
//     type name in every comment is noise.
//   - `manual_debug_impl` flags `JitModule`'s `Debug` impl
//     (which deliberately omits the JIT module pointer to keep
//     debug output stable across runs).
#![forbid(unsafe_code)]
#![allow(clippy::comparison_chain)]

use std::collections::HashMap;

use std::collections::HashSet;

use anyhow::{Result, anyhow, bail};
use cranelift_codegen::ir::{
    AbiParam, ExtFuncData, Function, GlobalValueData, InstBuilder, MemFlagsData, Signature,
    StackSlotData, StackSlotKind, UserExternalName, UserFuncName, condcodes::IntCC,
    immediates::Imm64, types,
};
use cranelift_codegen::isa::{CallConv, TargetFrontendConfig};
use cranelift_codegen::settings::{self, Configurable};
use cranelift_codegen::{Context, ir};
use cranelift_frontend::{FunctionBuilder, FunctionBuilderContext, Variable};
use cranelift_module::{DataDescription, DataId, FuncId, Linkage, Module, ModuleDeclarations};
use cranelift_object::{ObjectBuilder, ObjectModule};
use gossamer_mir::{
    BinOp, Body, ConstValue, Local, Operand, Place, Projection, Rvalue, StatementKind, Terminator,
    UnOp,
};
use gossamer_types::{FloatTy, IntTy, Ty, TyCtxt, TyKind};
use rayon::prelude::*;

use super::*;

use super::*;

pub(super) fn name_to_static(name: &str, set: &[&'static str]) -> Option<&'static str> {
    for s in set {
        if *s == name {
            return Some(*s);
        }
    }
    None
}

pub(super) fn generic_rt_static_name(name: &str) -> Option<&'static str> {
    if let Some(s) = name_to_static(
        name,
        &[
            "gos_rt_http_request_path",
            "gos_rt_http_request_method",
            "gos_rt_http_request_query",
            "gos_rt_http_request_peer_addr",
            "gos_rt_http_request_context",
            "gos_rt_http_request_headers",
            "gos_rt_http_request_body_str",
            "gos_rt_http_request_raw_body",
            "gos_rt_http_response_text_new",
            "gos_rt_http_response_json_new",
            "gos_rt_http_response_stream_new",
        ],
    ) {
        return Some(s);
    }
    match name {
        // 0.7.0 string surface.
        "gos_rt_str_split_once" => Some("gos_rt_str_split_once"),
        "gos_rt_str_rsplit_once" => Some("gos_rt_str_rsplit_once"),
        "gos_rt_str_count" => Some("gos_rt_str_count"),
        "gos_rt_str_chars" => Some("gos_rt_str_chars"),
        "gos_rt_str_strip_chars" => Some("gos_rt_str_strip_chars"),
        "gos_rt_str_lstrip_chars" => Some("gos_rt_str_lstrip_chars"),
        "gos_rt_str_rstrip_chars" => Some("gos_rt_str_rstrip_chars"),
        "gos_rt_str_zfill" => Some("gos_rt_str_zfill"),
        "gos_rt_str_center" => Some("gos_rt_str_center"),
        "gos_rt_str_slice" => Some("gos_rt_str_slice"),
        "gos_rt_str_clear" => Some("gos_rt_str_clear"),
        "gos_rt_str_with_capacity" => Some("gos_rt_str_with_capacity"),
        "gos_rt_str_truncate" => Some("gos_rt_str_truncate"),
        "gos_rt_str_rfind_opt" => Some("gos_rt_str_rfind_opt"),
        "gos_rt_str_push_char" => Some("gos_rt_str_push_char"),
        "gos_rt_str_push_byte" => Some("gos_rt_str_push_byte"),
        "gos_rt_deque_new" => Some("gos_rt_deque_new"),
        "gos_rt_deque_from_vec_i64" => Some("gos_rt_deque_from_vec_i64"),
        "gos_rt_set_from_vec_i64" => Some("gos_rt_set_from_vec_i64"),
        "gos_rt_set_from_vec_str" => Some("gos_rt_set_from_vec_str"),
        "gos_rt_btree_set_from_vec_i64" => Some("gos_rt_btree_set_from_vec_i64"),
        "gos_rt_btree_set_from_vec_str" => Some("gos_rt_btree_set_from_vec_str"),
        "gos_rt_queue_new" => Some("gos_rt_queue_new"),
        "gos_rt_queue_from_vec_i64" => Some("gos_rt_queue_from_vec_i64"),
        "gos_rt_stack_new" => Some("gos_rt_stack_new"),
        "gos_rt_stack_from_vec_i64" => Some("gos_rt_stack_from_vec_i64"),
        "gos_rt_deque_push_back" => Some("gos_rt_deque_push_back"),
        "gos_rt_bheap_max_format_desc" => Some("gos_rt_bheap_max_format_desc"),
        "gos_rt_bheap_max_from_vec_desc" => Some("gos_rt_bheap_max_from_vec_desc"),
        "gos_rt_bheap_max_pop_desc" => Some("gos_rt_bheap_max_pop_desc"),
        "gos_rt_bheap_max_push_desc" => Some("gos_rt_bheap_max_push_desc"),
        "gos_rt_bheap_min_format_desc" => Some("gos_rt_bheap_min_format_desc"),
        "gos_rt_bheap_min_from_vec_desc" => Some("gos_rt_bheap_min_from_vec_desc"),
        "gos_rt_bheap_min_pop_desc" => Some("gos_rt_bheap_min_pop_desc"),
        "gos_rt_bheap_min_push_desc" => Some("gos_rt_bheap_min_push_desc"),
        "gos_rt_bheap_new_typed" => Some("gos_rt_bheap_new_typed"),
        "gos_rt_bheap_peek_elem" => Some("gos_rt_bheap_peek_elem"),
        "gos_rt_desc_cmp" => Some("gos_rt_desc_cmp"),
        "gos_rt_map_eq" => Some("gos_rt_map_eq"),
        "gos_rt_dyn_nil" => Some("gos_rt_dyn_nil"),
        "gos_rt_dyn_bool" => Some("gos_rt_dyn_bool"),
        "gos_rt_dyn_int" => Some("gos_rt_dyn_int"),
        "gos_rt_dyn_float" => Some("gos_rt_dyn_float"),
        "gos_rt_dyn_char" => Some("gos_rt_dyn_char"),
        "gos_rt_dyn_string" => Some("gos_rt_dyn_string"),
        "gos_rt_dyn_bytes" => Some("gos_rt_dyn_bytes"),
        "gos_rt_dyn_list" => Some("gos_rt_dyn_list"),
        "gos_rt_dyn_map" => Some("gos_rt_dyn_map"),
        "gos_rt_dyn_tagged" => Some("gos_rt_dyn_tagged"),
        "gos_rt_dyn_kind" => Some("gos_rt_dyn_kind"),
        "gos_rt_dyn_name" => Some("gos_rt_dyn_name"),
        "gos_rt_dyn_kind_name" => Some("gos_rt_dyn_kind_name"),
        "gos_rt_dyn_len" => Some("gos_rt_dyn_len"),
        "gos_rt_dyn_at" => Some("gos_rt_dyn_at"),
        "gos_rt_dyn_arm_index" => Some("gos_rt_dyn_arm_index"),
        "gos_rt_dyn_field_i64" => Some("gos_rt_dyn_field_i64"),
        "gos_rt_dyn_field_f64" => Some("gos_rt_dyn_field_f64"),
        "gos_rt_dyn_field_str" => Some("gos_rt_dyn_field_str"),
        "gos_rt_dyn_field_dyn" => Some("gos_rt_dyn_field_dyn"),
        "gos_rt_dyn_key_at" => Some("gos_rt_dyn_key_at"),
        "gos_rt_dyn_as_i64" => Some("gos_rt_dyn_as_i64"),
        "gos_rt_dyn_as_f64" => Some("gos_rt_dyn_as_f64"),
        "gos_rt_dyn_as_bool" => Some("gos_rt_dyn_as_bool"),
        "gos_rt_dyn_as_char" => Some("gos_rt_dyn_as_char"),
        "gos_rt_dyn_as_str" => Some("gos_rt_dyn_as_str"),
        "gos_rt_dyn_as_bytes" => Some("gos_rt_dyn_as_bytes"),
        "gos_rt_dyn_clone" => Some("gos_rt_dyn_clone"),
        "gos_rt_dyn_free" => Some("gos_rt_dyn_free"),
        "gos_rt_dyn_eq" => Some("gos_rt_dyn_eq"),
        "gos_rt_dyn_format" => Some("gos_rt_dyn_format"),
        "gos_rt_dyn_from_binding_variant" => Some("gos_rt_dyn_from_binding_variant"),
        "gos_rt_set_eq" => Some("gos_rt_set_eq"),
        "gos_rt_binding_bytes_from_vec" => Some("gos_rt_binding_bytes_from_vec"),
        "gos_rt_binding_bytes_to_vec" => Some("gos_rt_binding_bytes_to_vec"),
        "gos_rt_binding_map_from_map" => Some("gos_rt_binding_map_from_map"),
        "gos_rt_binding_map_to_map" => Some("gos_rt_binding_map_to_map"),
        "gos_rt_binding_tuple_from_slots" => Some("gos_rt_binding_tuple_from_slots"),
        "gos_rt_binding_tuple_to_slots" => Some("gos_rt_binding_tuple_to_slots"),
        "gos_rt_binding_struct_from_slots" => Some("gos_rt_binding_struct_from_slots"),
        "gos_rt_binding_struct_to_slots" => Some("gos_rt_binding_struct_to_slots"),
        "gos_rt_set_format_tagged" => Some("gos_rt_set_format_tagged"),
        "gos_rt_set_insert_ekey" => Some("gos_rt_set_insert_ekey"),
        "gos_rt_set_contains_ekey" => Some("gos_rt_set_contains_ekey"),
        "gos_rt_set_remove_ekey" => Some("gos_rt_set_remove_ekey"),
        "gos_rt_set_to_vec_ekey" => Some("gos_rt_set_to_vec_ekey"),
        "gos_rt_set_format_ekey" => Some("gos_rt_set_format_ekey"),
        "gos_rt_deque_push_back_wide" => Some("gos_rt_deque_push_back_wide"),
        "gos_rt_deque_push_front_wide" => Some("gos_rt_deque_push_front_wide"),
        "gos_rt_deque_new_typed" => Some("gos_rt_deque_new_typed"),
        "gos_rt_deque_from_vec" => Some("gos_rt_deque_from_vec"),
        "gos_rt_deque_vec" => Some("gos_rt_deque_vec"),
        "gos_rt_deque_format_desc" => Some("gos_rt_deque_format_desc"),
        "gos_rt_queue_format_desc" => Some("gos_rt_queue_format_desc"),
        "gos_rt_stack_format_desc" => Some("gos_rt_stack_format_desc"),
        "gos_rt_deque_push_back_f64" => Some("gos_rt_deque_push_back_f64"),
        "gos_rt_deque_push_front" => Some("gos_rt_deque_push_front"),
        "gos_rt_deque_push_front_f64" => Some("gos_rt_deque_push_front_f64"),
        "gos_rt_deque_pop_front" => Some("gos_rt_deque_pop_front"),
        "gos_rt_deque_pop_back" => Some("gos_rt_deque_pop_back"),
        "gos_rt_deque_peek_front" => Some("gos_rt_deque_peek_front"),
        "gos_rt_deque_peek_back" => Some("gos_rt_deque_peek_back"),
        "gos_rt_deque_len" => Some("gos_rt_deque_len"),
        "gos_rt_deque_is_empty" => Some("gos_rt_deque_is_empty"),
        "gos_rt_set_is_empty" => Some("gos_rt_set_is_empty"),
        "gos_rt_deque_clear" => Some("gos_rt_deque_clear"),
        "gos_rt_deque_free" => Some("gos_rt_deque_free"),
        "gos_rt_strings_join" => Some("gos_rt_strings_join"),
        "gos_rt_io_copy" => Some("gos_rt_io_copy"),
        "gos_rt_io_string_reader" => Some("gos_rt_io_string_reader"),
        "gos_rt_io_buffer_writer" => Some("gos_rt_io_buffer_writer"),
        "gos_rt_io_limit_reader" => Some("gos_rt_io_limit_reader"),
        "gos_rt_io_tee_reader" => Some("gos_rt_io_tee_reader"),
        "gos_rt_io_multi_reader" => Some("gos_rt_io_multi_reader"),
        "gos_rt_io_pipe" => Some("gos_rt_io_pipe"),
        "gos_rt_io_copy_n" => Some("gos_rt_io_copy_n"),
        "gos_rt_io_drain" => Some("gos_rt_io_drain"),
        "gos_rt_io_contents" => Some("gos_rt_io_contents"),
        "gos_rt_io_write_str" => Some("gos_rt_io_write_str"),
        "gos_rt_io_close_writer" => Some("gos_rt_io_close_writer"),
        "gos_rt_io_read_all" => Some("gos_rt_io_read_all"),
        "gos_rt_sandbox_policy_new" => Some("gos_rt_sandbox_policy_new"),
        "gos_rt_sandbox_policy_command_default" => Some("gos_rt_sandbox_policy_command_default"),
        "gos_rt_sandbox_policy_read_write" => Some("gos_rt_sandbox_policy_read_write"),
        "gos_rt_sandbox_policy_read_only" => Some("gos_rt_sandbox_policy_read_only"),
        "gos_rt_sandbox_policy_deny" => Some("gos_rt_sandbox_policy_deny"),
        "gos_rt_sandbox_policy_env_allow" => Some("gos_rt_sandbox_policy_env_allow"),
        "gos_rt_sandbox_policy_env_set" => Some("gos_rt_sandbox_policy_env_set"),
        "gos_rt_sandbox_policy_level" => Some("gos_rt_sandbox_policy_level"),
        "gos_rt_sandbox_policy_working_directory" => {
            Some("gos_rt_sandbox_policy_working_directory")
        }
        "gos_rt_sandbox_run" => Some("gos_rt_sandbox_run"),
        "gos_rt_sandbox_max_level" => Some("gos_rt_sandbox_max_level"),
        "gos_rt_sandbox_platform" => Some("gos_rt_sandbox_platform"),
        "gos_rt_sandbox_capabilities_json" => Some("gos_rt_sandbox_capabilities_json"),
        "gos_rt_sandbox_notes" => Some("gos_rt_sandbox_notes"),
        "gos_rt_sandbox_policy_network_mode" => Some("gos_rt_sandbox_policy_network_mode"),
        "gos_rt_sandbox_policy_for_fetch_phase" => Some("gos_rt_sandbox_policy_for_fetch_phase"),
        "gos_rt_sandbox_policy_read_only_cwd" => Some("gos_rt_sandbox_policy_read_only_cwd"),
        "gos_rt_sandbox_policy_temp" => Some("gos_rt_sandbox_policy_temp"),
        "gos_rt_sandbox_policy_check" => Some("gos_rt_sandbox_policy_check"),
        "gos_rt_sandbox_policy_mechanisms" => Some("gos_rt_sandbox_policy_mechanisms"),
        "gos_rt_sandbox_policy_to_json" => Some("gos_rt_sandbox_policy_to_json"),
        "gos_rt_sandbox_policy_access" => Some("gos_rt_sandbox_policy_access"),
        "gos_rt_sandbox_policy_read_write_grants" => {
            Some("gos_rt_sandbox_policy_read_write_grants")
        }
        "gos_rt_sandbox_policy_read_only_grants" => Some("gos_rt_sandbox_policy_read_only_grants"),
        "gos_rt_sandbox_policy_denials" => Some("gos_rt_sandbox_policy_denials"),
        "gos_rt_sandbox_policy_environment_names" => {
            Some("gos_rt_sandbox_policy_environment_names")
        }
        "gos_rt_sandbox_policy_environment_value" => {
            Some("gos_rt_sandbox_policy_environment_value")
        }
        "gos_rt_sandbox_policy_level_name" => Some("gos_rt_sandbox_policy_level_name"),
        "gos_rt_sandbox_policy_network_name" => Some("gos_rt_sandbox_policy_network_name"),
        "gos_rt_sandbox_policy_working_directory_path" => {
            Some("gos_rt_sandbox_policy_working_directory_path")
        }
        "gos_rt_sandbox_policy_level_blocker" => Some("gos_rt_sandbox_policy_level_blocker"),
        "gos_rt_sandbox_policy_network_enforcement_kind" => {
            Some("gos_rt_sandbox_policy_network_enforcement_kind")
        }
        "gos_rt_sandbox_policy_network_enforcement_reason" => {
            Some("gos_rt_sandbox_policy_network_enforcement_reason")
        }
        "gos_rt_sandbox_os_description" => Some("gos_rt_sandbox_os_description"),
        "gos_rt_sandbox_filesystem_kind" => Some("gos_rt_sandbox_filesystem_kind"),
        "gos_rt_sandbox_filesystem_reason" => Some("gos_rt_sandbox_filesystem_reason"),
        "gos_rt_sandbox_network_kind" => Some("gos_rt_sandbox_network_kind"),
        "gos_rt_sandbox_network_reason" => Some("gos_rt_sandbox_network_reason"),
        "gos_rt_sandbox_process_isolation_kind" => Some("gos_rt_sandbox_process_isolation_kind"),
        "gos_rt_sandbox_process_isolation_reason" => {
            Some("gos_rt_sandbox_process_isolation_reason")
        }
        "gos_rt_sandbox_env_never_passed" => Some("gos_rt_sandbox_env_never_passed"),
        "gos_rt_sandbox_policy_env_allow_all" => Some("gos_rt_sandbox_policy_env_allow_all"),
        "gos_rt_sandbox_expand" => Some("gos_rt_sandbox_expand"),
        "gos_rt_sandbox_prefix_of" => Some("gos_rt_sandbox_prefix_of"),
        "gos_rt_sandbox_resolve_on_path" => Some("gos_rt_sandbox_resolve_on_path"),
        "gos_rt_sandbox_home_directory" => Some("gos_rt_sandbox_home_directory"),
        "gos_rt_sandbox_exit_policy_error" => Some("gos_rt_sandbox_exit_policy_error"),
        "gos_rt_sandbox_exit_command_not_found" => Some("gos_rt_sandbox_exit_command_not_found"),
        "gos_rt_sandbox_exit_level_unavailable" => Some("gos_rt_sandbox_exit_level_unavailable"),
        "gos_rt_sandbox_exit_signal_base" => Some("gos_rt_sandbox_exit_signal_base"),
        "gos_rt_sandbox_run_inherit" => Some("gos_rt_sandbox_run_inherit"),
        "gos_rt_uuid_v4" => Some("gos_rt_uuid_v4"),
        "gos_rt_uuid_v7" => Some("gos_rt_uuid_v7"),
        "gos_rt_uuid_is_valid" => Some("gos_rt_uuid_is_valid"),
        "gos_rt_uuid_normalize" => Some("gos_rt_uuid_normalize"),
        "gos_rt_uuid_simple" => Some("gos_rt_uuid_simple"),
        "gos_rt_path_glob" => Some("gos_rt_path_glob"),
        "gos_rt_path_matches" => Some("gos_rt_path_matches"),
        "gos_rt_sort_stable_i64" => Some("gos_rt_sort_stable_i64"),
        "gos_rt_sort_stable_f64" => Some("gos_rt_sort_stable_f64"),
        "gos_rt_sort_binary_search_f64" => Some("gos_rt_sort_binary_search_f64"),
        "gos_rt_sort_partition_point_f64" => Some("gos_rt_sort_partition_point_f64"),
        "gos_rt_sort_stable_str" => Some("gos_rt_sort_stable_str"),
        "gos_rt_sort_binary_search_i64" => Some("gos_rt_sort_binary_search_i64"),
        "gos_rt_sort_binary_search_str" => Some("gos_rt_sort_binary_search_str"),
        "gos_rt_sort_partition_point_i64" => Some("gos_rt_sort_partition_point_i64"),
        "gos_rt_sort_partition_point_str" => Some("gos_rt_sort_partition_point_str"),
        "gos_rt_path_base" => Some("gos_rt_path_base"),
        "gos_rt_path_components" => Some("gos_rt_path_components"),
        "gos_rt_path_prefixes" => Some("gos_rt_path_prefixes"),
        "gos_rt_path_unique_prefixes" => Some("gos_rt_path_unique_prefixes"),
        "gos_rt_path_dir" => Some("gos_rt_path_dir"),
        "gos_rt_path_ext" => Some("gos_rt_path_ext"),
        "gos_rt_path_file_name" => Some("gos_rt_path_file_name"),
        "gos_rt_path_parent" => Some("gos_rt_path_parent"),
        "gos_rt_path_stem" => Some("gos_rt_path_stem"),
        "gos_rt_vec_first" => Some("gos_rt_vec_first"),
        "gos_rt_vec_pop_opt" => Some("gos_rt_vec_pop_opt"),
        "gos_rt_vec_last" => Some("gos_rt_vec_last"),
        "gos_rt_vec_get_opt" => Some("gos_rt_vec_get_opt"),
        "gos_rt_vec_reversed" => Some("gos_rt_vec_reversed"),
        "gos_rt_vec_index_of_i64" => Some("gos_rt_vec_index_of_i64"),
        "gos_rt_vec_index_of_str" => Some("gos_rt_vec_index_of_str"),
        "gos_rt_vec_count_of_i64" => Some("gos_rt_vec_count_of_i64"),
        "gos_rt_vec_count_of_str" => Some("gos_rt_vec_count_of_str"),
        "gos_rt_vec_contains_i64" => Some("gos_rt_vec_contains_i64"),
        "gos_rt_vec_contains_str" => Some("gos_rt_vec_contains_str"),
        "gos_rt_vec_slice_result" => Some("gos_rt_vec_slice_result"),
        "gos_rt_vec_insert_safe" => Some("gos_rt_vec_insert_safe"),
        "gos_rt_vec_remove_at" => Some("gos_rt_vec_remove_at"),
        "gos_rt_vec_remove_safe" => Some("gos_rt_vec_remove_safe"),
        "gos_rt_vec_swap_safe" => Some("gos_rt_vec_swap_safe"),
        "gos_rt_vec_clear" => Some("gos_rt_vec_clear"),
        "gos_rt_vec_capacity" => Some("gos_rt_vec_capacity"),
        "gos_rt_vec_extend" => Some("gos_rt_vec_extend"),
        "gos_rt_vec_truncate" => Some("gos_rt_vec_truncate"),
        "gos_rt_map_keys_vec" => Some("gos_rt_map_keys_vec"),
        "gos_rt_map_values_vec" => Some("gos_rt_map_values_vec"),
        "gos_rt_map_pop_i64" => Some("gos_rt_map_pop_i64"),
        "gos_rt_map_pop_str" => Some("gos_rt_map_pop_str"),
        "gos_rt_map_pop_typed_str" => Some("gos_rt_map_pop_typed_str"),
        "gos_rt_map_get_typed_str_opt" => Some("gos_rt_map_get_typed_str_opt"),
        "gos_rt_map_insert_i64_i64_opt" => Some("gos_rt_map_insert_i64_i64_opt"),
        "gos_rt_map_insert_i64_str_opt" => Some("gos_rt_map_insert_i64_str_opt"),
        "gos_rt_map_insert_str_i64_opt" => Some("gos_rt_map_insert_str_i64_opt"),
        "gos_rt_map_insert_typed_str_i64" => Some("gos_rt_map_insert_typed_str_i64"),
        "gos_rt_map_insert_typed_str_i64_opt" => Some("gos_rt_map_insert_typed_str_i64_opt"),
        "gos_rt_map_insert_str_str_opt" => Some("gos_rt_map_insert_str_str_opt"),
        "gos_rt_map_insert_skey_opt" => Some("gos_rt_map_insert_skey_opt"),
        "gos_rt_map_get_skey_opt" => Some("gos_rt_map_get_skey_opt"),
        "gos_rt_map_pop_skey" => Some("gos_rt_map_pop_skey"),
        "gos_rt_map_contains_skey" => Some("gos_rt_map_contains_skey"),
        "gos_rt_map_get_or_skey" => Some("gos_rt_map_get_or_skey"),
        "gos_rt_map_or_insert_skey" => Some("gos_rt_map_or_insert_skey"),
        "gos_rt_map_inc_skey" => Some("gos_rt_map_inc_skey"),
        "gos_rt_map_insert_ekey_opt" => Some("gos_rt_map_insert_ekey_opt"),
        "gos_rt_map_get_ekey_opt" => Some("gos_rt_map_get_ekey_opt"),
        "gos_rt_map_contains_ekey" => Some("gos_rt_map_contains_ekey"),
        "gos_rt_map_pop_ekey" => Some("gos_rt_map_pop_ekey"),
        "gos_rt_map_get_or_ekey" => Some("gos_rt_map_get_or_ekey"),
        "gos_rt_map_or_insert_ekey" => Some("gos_rt_map_or_insert_ekey"),
        "gos_rt_map_inc_ekey" => Some("gos_rt_map_inc_ekey"),
        "gos_rt_map_keys_ekey" => Some("gos_rt_map_keys_ekey"),
        "gos_rt_min_i64" => Some("gos_rt_min_i64"),
        "gos_rt_max_i64" => Some("gos_rt_max_i64"),
        "gos_rt_clamp_i64" => Some("gos_rt_clamp_i64"),
        "gos_rt_min_f64" => Some("gos_rt_min_f64"),
        "gos_rt_max_f64" => Some("gos_rt_max_f64"),
        "gos_rt_clamp_f64" => Some("gos_rt_clamp_f64"),
        "gos_rt_error_new" => Some("gos_rt_error_new"),
        "gos_rt_error_from" => Some("gos_rt_error_from"),
        "gos_rt_error_wrap" => Some("gos_rt_error_wrap"),
        "gos_rt_error_message" => Some("gos_rt_error_message"),
        "gos_rt_error_display" => Some("gos_rt_error_display"),
        "gos_rt_error_cause" => Some("gos_rt_error_cause"),
        "gos_rt_error_is" => Some("gos_rt_error_is"),
        "gos_rt_error_is_sentinel" => Some("gos_rt_error_is_sentinel"),
        "gos_rt_error_chain" => Some("gos_rt_error_chain"),
        "gos_rt_error_field" => Some("gos_rt_error_field"),
        "gos_rt_error_fields" => Some("gos_rt_error_fields"),
        "gos_rt_error_with_field" => Some("gos_rt_error_with_field"),
        "gos_rt_regex_compile" => Some("gos_rt_regex_compile"),
        "gos_rt_regex_is_match" => Some("gos_rt_regex_is_match"),
        "gos_rt_regex_find" => Some("gos_rt_regex_find"),
        "gos_rt_regex_find_opt" => Some("gos_rt_regex_find_opt"),
        "gos_rt_regex_captures" => Some("gos_rt_regex_captures"),
        "gos_rt_regex_find_all" => Some("gos_rt_regex_find_all"),
        "gos_rt_regex_captures_all" => Some("gos_rt_regex_captures_all"),
        "gos_rt_regex_replace" => Some("gos_rt_regex_replace"),
        "gos_rt_regex_replace_all" => Some("gos_rt_regex_replace_all"),
        "gos_rt_regex_split" => Some("gos_rt_regex_split"),
        "gos_rt_fs_read_to_string" => Some("gos_rt_fs_read_to_string"),
        "gos_rt_fs_write" => Some("gos_rt_fs_write"),
        "gos_rt_fs_create_dir_all" => Some("gos_rt_fs_create_dir_all"),
        "gos_rt_fs_create_dir_all_mode" => Some("gos_rt_fs_create_dir_all_mode"),
        "gos_rt_fs_create_dir_mode" => Some("gos_rt_fs_create_dir_mode"),
        "gos_rt_fs_permissions" => Some("gos_rt_fs_permissions"),
        "gos_rt_fs_set_permissions" => Some("gos_rt_fs_set_permissions"),
        "gos_rt_fs_write_mode" => Some("gos_rt_fs_write_mode"),
        "gos_rt_fs_file_close" => Some("gos_rt_fs_file_close"),
        "gos_rt_fs_file_create" => Some("gos_rt_fs_file_create"),
        "gos_rt_fs_file_flush" => Some("gos_rt_fs_file_flush"),
        "gos_rt_fs_file_open" => Some("gos_rt_fs_file_open"),
        "gos_rt_fs_file_read" => Some("gos_rt_fs_file_read"),
        "gos_rt_fs_file_read_to_string" => Some("gos_rt_fs_file_read_to_string"),
        "gos_rt_fs_file_write" => Some("gos_rt_fs_file_write"),
        "gos_rt_fs_temp_dir" => Some("gos_rt_fs_temp_dir"),
        "gos_rt_fs_temp_file" => Some("gos_rt_fs_temp_file"),
        "gos_rt_fs_open_options_append" => Some("gos_rt_fs_open_options_append"),
        "gos_rt_fs_open_options_create" => Some("gos_rt_fs_open_options_create"),
        "gos_rt_fs_open_options_create_new" => Some("gos_rt_fs_open_options_create_new"),
        "gos_rt_fs_open_options_new" => Some("gos_rt_fs_open_options_new"),
        "gos_rt_fs_open_options_open" => Some("gos_rt_fs_open_options_open"),
        "gos_rt_fs_open_options_read" => Some("gos_rt_fs_open_options_read"),
        "gos_rt_fs_open_options_truncate" => Some("gos_rt_fs_open_options_truncate"),
        "gos_rt_fs_open_options_write" => Some("gos_rt_fs_open_options_write"),
        "gos_rt_path_join" => Some("gos_rt_path_join"),
        "gos_rt_flag_set_new" => Some("gos_rt_flag_set_new"),
        "gos_rt_flag_set_string" => Some("gos_rt_flag_set_string"),
        "gos_rt_flag_set_int" => Some("gos_rt_flag_set_int"),
        "gos_rt_flag_set_uint" => Some("gos_rt_flag_set_uint"),
        "gos_rt_flag_set_float" => Some("gos_rt_flag_set_float"),
        "gos_rt_flag_set_bool" => Some("gos_rt_flag_set_bool"),
        "gos_rt_flag_set_duration" => Some("gos_rt_flag_set_duration"),
        "gos_rt_flag_set_string_list" => Some("gos_rt_flag_set_string_list"),
        "gos_rt_flag_set_short" => Some("gos_rt_flag_set_short"),
        "gos_rt_flag_set_usage" => Some("gos_rt_flag_set_usage"),
        "gos_rt_flag_set_parse" => Some("gos_rt_flag_set_parse"),
        "gos_rt_duration_from_secs" => Some("gos_rt_duration_from_secs"),
        "gos_rt_duration_from_millis" => Some("gos_rt_duration_from_millis"),
        "gos_rt_time_format_rfc3339" => Some("gos_rt_time_format_rfc3339"),
        "gos_rt_time_parse_rfc3339" => Some("gos_rt_time_parse_rfc3339"),
        "gos_rt_time_add_date_raw" => Some("gos_rt_time_add_date_raw"),
        "gos_rt_time_civil_raw" => Some("gos_rt_time_civil_raw"),
        "gos_rt_time_fixed_location_raw" => Some("gos_rt_time_fixed_location_raw"),
        "gos_rt_time_format_in_raw" => Some("gos_rt_time_format_in_raw"),
        "gos_rt_time_location_raw" => Some("gos_rt_time_location_raw"),
        "gos_rt_time_resolve_raw" => Some("gos_rt_time_resolve_raw"),
        "gos_rt_flag_parse" => Some("gos_rt_flag_parse"),
        "gos_rt_flag_map_get" => Some("gos_rt_flag_map_get"),
        "gos_rt_os_env" => Some("gos_rt_os_env"),
        "gos_rt_os_program_name" => Some("gos_rt_os_program_name"),
        "gos_rt_env_temp_dir" => Some("gos_rt_env_temp_dir"),
        "gos_rt_env_home_dir" => Some("gos_rt_env_home_dir"),
        "gos_rt_env_vars" => Some("gos_rt_env_vars"),
        "gos_rt_os_cwd" => Some("gos_rt_os_cwd"),
        "gos_rt_os_exists" => Some("gos_rt_os_exists"),
        "gos_rt_os_is_file" => Some("gos_rt_os_is_file"),
        "gos_rt_os_is_dir" => Some("gos_rt_os_is_dir"),
        "gos_rt_os_is_symlink" => Some("gos_rt_os_is_symlink"),
        "gos_rt_os_file_size" => Some("gos_rt_os_file_size"),
        "gos_rt_os_remove_file" => Some("gos_rt_os_remove_file"),
        "gos_rt_result_map_bare" => Some("gos_rt_result_map_bare"),
        "gos_rt_result_map_err_bare" => Some("gos_rt_result_map_err_bare"),
        "gos_rt_os_write_file_result" => Some("gos_rt_os_write_file_result"),
        "gos_rt_os_write_file_bytes_result" => Some("gos_rt_os_write_file_bytes_result"),
        "gos_rt_fs_read_bytes_result" => Some("gos_rt_fs_read_bytes_result"),
        "gos_rt_os_mkdir_all_result" => Some("gos_rt_os_mkdir_all_result"),
        "gos_rt_os_remove_file_result" => Some("gos_rt_os_remove_file_result"),
        "gos_rt_os_remove_dir_all_result" => Some("gos_rt_os_remove_dir_all_result"),
        "gos_rt_http_stream" => Some("gos_rt_http_stream"),
        "gos_rt_http_get" => Some("gos_rt_http_get"),
        "gos_rt_http_head" => Some("gos_rt_http_head"),
        "gos_rt_http_options" => Some("gos_rt_http_options"),
        "gos_rt_http_post" => Some("gos_rt_http_post"),
        "gos_rt_http_put" => Some("gos_rt_http_put"),
        "gos_rt_http_delete" => Some("gos_rt_http_delete"),
        "gos_rt_http_request" => Some("gos_rt_http_request"),
        "gos_rt_http_request_bytes" => Some("gos_rt_http_request_bytes"),
        "gos_rt_http_stream_next_line" => Some("gos_rt_http_stream_next_line"),
        "gos_rt_http_stream_next_chunk" => Some("gos_rt_http_stream_next_chunk"),
        "gos_rt_fs_list_dir" => Some("gos_rt_fs_list_dir"),
        "gos_rt_fs_walk_dir" => Some("gos_rt_fs_walk_dir"),
        "gos_rt_exec_run" => Some("gos_rt_exec_run"),
        "gos_rt_exec_run_in" => Some("gos_rt_exec_run_in"),
        "gos_rt_exec_spawn" => Some("gos_rt_exec_spawn"),
        "gos_rt_exec_spawn_piped" => Some("gos_rt_exec_spawn_piped"),
        "gos_rt_child_write_stdin" => Some("gos_rt_child_write_stdin"),
        "gos_rt_child_close_stdin" => Some("gos_rt_child_close_stdin"),
        "gos_rt_child_read_line" => Some("gos_rt_child_read_line"),
        "gos_rt_stream_next_line" => Some("gos_rt_stream_next_line"),
        "gos_rt_child_read_stdout" => Some("gos_rt_child_read_stdout"),
        "gos_rt_child_wait" => Some("gos_rt_child_wait"),
        "gos_rt_child_kill" => Some("gos_rt_child_kill"),
        "gos_rt_exec_kill" => Some("gos_rt_exec_kill"),
        "gos_rt_exec_signal" => Some("gos_rt_exec_signal"),
        "gos_rt_exec_kill_group" => Some("gos_rt_exec_kill_group"),
        "gos_rt_exec_wait_timeout" => Some("gos_rt_exec_wait_timeout"),
        "gos_rt_exec_pipeline_run" => Some("gos_rt_exec_pipeline_run"),
        "gos_rt_signal_on" => Some("gos_rt_signal_on"),
        "gos_rt_signal_wait" => Some("gos_rt_signal_wait"),
        "gos_rt_signal_try_wait" => Some("gos_rt_signal_try_wait"),
        "gos_rt_os_set_env" => Some("gos_rt_os_set_env"),
        "gos_rt_os_unset_env" => Some("gos_rt_os_unset_env"),
        "gos_rt_os_user_current_name" => Some("gos_rt_os_user_current_name"),
        "gos_rt_os_user_current_uid" => Some("gos_rt_os_user_current_uid"),
        "gos_rt_os_user_current_gid" => Some("gos_rt_os_user_current_gid"),
        "gos_rt_os_user_current_home" => Some("gos_rt_os_user_current_home"),
        "gos_rt_os_user_lookup_uid" => Some("gos_rt_os_user_lookup_uid"),
        "gos_rt_os_user_lookup_name" => Some("gos_rt_os_user_lookup_name"),
        "gos_rt_netip_is_valid" => Some("gos_rt_netip_is_valid"),
        "gos_rt_netip_is_v4" => Some("gos_rt_netip_is_v4"),
        "gos_rt_netip_is_v6" => Some("gos_rt_netip_is_v6"),
        "gos_rt_netip_is_loopback" => Some("gos_rt_netip_is_loopback"),
        "gos_rt_netip_is_unspecified" => Some("gos_rt_netip_is_unspecified"),
        "gos_rt_netip_is_multicast" => Some("gos_rt_netip_is_multicast"),
        "gos_rt_netip_is_private" => Some("gos_rt_netip_is_private"),
        "gos_rt_netip_normalize" => Some("gos_rt_netip_normalize"),
        "gos_rt_netip_host_of" => Some("gos_rt_netip_host_of"),
        "gos_rt_netip_port_of" => Some("gos_rt_netip_port_of"),
        "gos_rt_netip_join_addr_port" => Some("gos_rt_netip_join_addr_port"),
        "gos_rt_mime_parse" => Some("gos_rt_mime_parse"),
        "gos_rt_mime_top" => Some("gos_rt_mime_top"),
        "gos_rt_mime_sub" => Some("gos_rt_mime_sub"),
        "gos_rt_mime_charset" => Some("gos_rt_mime_charset"),
        "gos_rt_mime_boundary" => Some("gos_rt_mime_boundary"),
        "gos_rt_mime_param" => Some("gos_rt_mime_param"),
        "gos_rt_mime_type_by_extension" => Some("gos_rt_mime_type_by_extension"),
        "gos_rt_mime_extension_by_type" => Some("gos_rt_mime_extension_by_type"),
        "gos_rt_mime_is_valid" => Some("gos_rt_mime_is_valid"),
        "gos_rt_toml_to_json" => Some("gos_rt_toml_to_json"),
        "gos_rt_toml_from_json" => Some("gos_rt_toml_from_json"),
        "gos_rt_toml_is_valid" => Some("gos_rt_toml_is_valid"),
        "gos_rt_toml_pretty" => Some("gos_rt_toml_pretty"),
        "gos_rt_yaml_to_json" => Some("gos_rt_yaml_to_json"),
        "gos_rt_yaml_from_json" => Some("gos_rt_yaml_from_json"),
        "gos_rt_yaml_is_valid" => Some("gos_rt_yaml_is_valid"),
        "gos_rt_sync_map_new" => Some("gos_rt_sync_map_new"),
        "gos_rt_sync_map_set" => Some("gos_rt_sync_map_set"),
        "gos_rt_sync_map_get" => Some("gos_rt_sync_map_get"),
        "gos_rt_sync_map_delete" => Some("gos_rt_sync_map_delete"),
        "gos_rt_sync_map_len" => Some("gos_rt_sync_map_len"),
        "gos_rt_sync_map_contains" => Some("gos_rt_sync_map_contains"),
        "gos_rt_sync_map_keys" => Some("gos_rt_sync_map_keys"),
        "gos_rt_barrier_new" => Some("gos_rt_barrier_new"),
        "gos_rt_barrier_wait" => Some("gos_rt_barrier_wait"),
        "gos_rt_once_new" => Some("gos_rt_once_new"),
        "gos_rt_once_call" => Some("gos_rt_once_call"),
        "gos_rt_math_rng_new" => Some("gos_rt_math_rng_new"),
        "gos_rt_math_rng_next_f64" => Some("gos_rt_math_rng_next_f64"),
        "gos_rt_math_rng_next_u32" => Some("gos_rt_math_rng_next_u32"),
        "gos_rt_math_rng_next_u64" => Some("gos_rt_math_rng_next_u64"),
        "gos_rt_math_rng_range_u64" => Some("gos_rt_math_rng_range_u64"),
        "gos_rt_bytes_builder_new" => Some("gos_rt_bytes_builder_new"),
        "gos_rt_bytes_builder_with_capacity" => Some("gos_rt_bytes_builder_with_capacity"),
        "gos_rt_bytes_builder_write" => Some("gos_rt_bytes_builder_write"),
        "gos_rt_bytes_builder_write_char" => Some("gos_rt_bytes_builder_write_char"),
        "gos_rt_bytes_builder_build" => Some("gos_rt_bytes_builder_build"),
        "gos_rt_bytes_builder_as_str" => Some("gos_rt_bytes_builder_as_str"),
        "gos_rt_bytes_builder_len" => Some("gos_rt_bytes_builder_len"),
        "gos_rt_bytes_buffer_new" => Some("gos_rt_bytes_buffer_new"),
        "gos_rt_bytes_buffer_with_capacity" => Some("gos_rt_bytes_buffer_with_capacity"),
        "gos_rt_bytes_buffer_write_str" => Some("gos_rt_bytes_buffer_write_str"),
        "gos_rt_bytes_buffer_push" => Some("gos_rt_bytes_buffer_push"),
        "gos_rt_bytes_buffer_len" => Some("gos_rt_bytes_buffer_len"),
        "gos_rt_bytes_buffer_is_empty" => Some("gos_rt_bytes_buffer_is_empty"),
        "gos_rt_bytes_buffer_clear" => Some("gos_rt_bytes_buffer_clear"),
        "gos_rt_bytes_buffer_to_string" => Some("gos_rt_bytes_buffer_to_string"),
        "gos_rt_bytes_split" => Some("gos_rt_bytes_split"),
        "gos_rt_bytes_replace" => Some("gos_rt_bytes_replace"),
        "gos_rt_net_ip_octets" => Some("gos_rt_net_ip_octets"),
        "gos_rt_tcp_listener_close" => Some("gos_rt_tcp_listener_close"),
        "gos_rt_tcp_stream_close" => Some("gos_rt_tcp_stream_close"),
        "gos_rt_tcp_stream_clear_read_timeout" => Some("gos_rt_tcp_stream_clear_read_timeout"),
        "gos_rt_tcp_stream_clear_write_timeout" => Some("gos_rt_tcp_stream_clear_write_timeout"),
        "gos_rt_tcp_stream_set_read_timeout_ms" => Some("gos_rt_tcp_stream_set_read_timeout_ms"),
        "gos_rt_tcp_stream_set_write_timeout_ms" => Some("gos_rt_tcp_stream_set_write_timeout_ms"),
        "gos_rt_udp_close" => Some("gos_rt_udp_close"),
        "gos_rt_field_error_new" => Some("gos_rt_field_error_new"),
        "gos_rt_field_error_path" => Some("gos_rt_field_error_path"),
        "gos_rt_field_error_message" => Some("gos_rt_field_error_message"),
        "gos_rt_field_error_code" => Some("gos_rt_field_error_code"),
        "gos_rt_validate_errors_new" => Some("gos_rt_validate_errors_new"),
        "gos_rt_validate_errors_add" => Some("gos_rt_validate_errors_add"),
        "gos_rt_validate_errors_is_empty" => Some("gos_rt_validate_errors_is_empty"),
        "gos_rt_validate_errors_len" => Some("gos_rt_validate_errors_len"),
        "gos_rt_validate_errors_count" => Some("gos_rt_validate_errors_count"),
        "gos_rt_validate_errors_get" => Some("gos_rt_validate_errors_get"),
        "gos_rt_validate_errors_collect" => Some("gos_rt_validate_errors_collect"),
        "gos_rt_rwlock_new" => Some("gos_rt_rwlock_new"),
        "gos_rt_rwlock_get" => Some("gos_rt_rwlock_get"),
        "gos_rt_rwlock_set" => Some("gos_rt_rwlock_set"),
        "gos_rt_rwlock_with_read" => Some("gos_rt_rwlock_with_read"),
        "gos_rt_rwlock_with_write" => Some("gos_rt_rwlock_with_write"),
        "gos_rt_shared_new" => Some("gos_rt_shared_new"),
        "gos_rt_shared_get" => Some("gos_rt_shared_get"),
        "gos_rt_shared_set" => Some("gos_rt_shared_set"),
        "gos_rt_shared_with" => Some("gos_rt_shared_with"),
        "gos_rt_shared_update" => Some("gos_rt_shared_update"),
        "gos_rt_ctx_background" => Some("gos_rt_ctx_background"),
        "gos_rt_ctx_with_cancel" => Some("gos_rt_ctx_with_cancel"),
        "gos_rt_ctx_with_timeout" => Some("gos_rt_ctx_with_timeout"),
        "gos_rt_ctx_cancel" => Some("gos_rt_ctx_cancel"),
        "gos_rt_ctx_cancelled" => Some("gos_rt_ctx_cancelled"),
        "gos_rt_ctx_is_cancelled" => Some("gos_rt_ctx_is_cancelled"),
        "gos_rt_ctx_done" => Some("gos_rt_ctx_done"),
        "gos_rt_metrics_counter_new" => Some("gos_rt_metrics_counter_new"),
        "gos_rt_metrics_counter_inc" => Some("gos_rt_metrics_counter_inc"),
        "gos_rt_metrics_counter_value" => Some("gos_rt_metrics_counter_value"),
        "gos_rt_metrics_gauge_new" => Some("gos_rt_metrics_gauge_new"),
        "gos_rt_metrics_gauge_set" => Some("gos_rt_metrics_gauge_set"),
        "gos_rt_metrics_gauge_inc" => Some("gos_rt_metrics_gauge_inc"),
        "gos_rt_metrics_gauge_dec" => Some("gos_rt_metrics_gauge_dec"),
        "gos_rt_metrics_gauge_value" => Some("gos_rt_metrics_gauge_value"),
        "gos_rt_metrics_histogram_new" => Some("gos_rt_metrics_histogram_new"),
        "gos_rt_metrics_histogram_observe" => Some("gos_rt_metrics_histogram_observe"),
        "gos_rt_metrics_histogram_sum" => Some("gos_rt_metrics_histogram_sum"),
        "gos_rt_metrics_histogram_count" => Some("gos_rt_metrics_histogram_count"),
        "gos_rt_metrics_registry_new" => Some("gos_rt_metrics_registry_new"),
        "gos_rt_metrics_registry_register" => Some("gos_rt_metrics_registry_register"),
        "gos_rt_metrics_registry_render" => Some("gos_rt_metrics_registry_render"),
        "gos_rt_middleware_new" => Some("gos_rt_middleware_new"),
        "gos_rt_middleware_new_kind" => Some("gos_rt_middleware_new_kind"),
        "gos_rt_mw_cors_permissive" => Some("gos_rt_mw_cors_permissive"),
        "gos_rt_mw_cors_new" => Some("gos_rt_mw_cors_new"),
        "gos_rt_mw_hsts_safe_default" => Some("gos_rt_mw_hsts_safe_default"),
        "gos_rt_mw_hsts_strict" => Some("gos_rt_mw_hsts_strict"),
        "gos_rt_mw_security_strict" => Some("gos_rt_mw_security_strict"),
        "gos_rt_mw_security_off" => Some("gos_rt_mw_security_off"),
        "gos_rt_mw_cache_no_store" => Some("gos_rt_mw_cache_no_store"),
        "gos_rt_mw_cache_immutable_for" => Some("gos_rt_mw_cache_immutable_for"),
        "gos_rt_mw_rate_limit_per_ip" => Some("gos_rt_mw_rate_limit_per_ip"),
        "gos_rt_middleware_serve" => Some("gos_rt_middleware_serve"),
        "gos_rt_trace_tracer_new" => Some("gos_rt_trace_tracer_new"),
        "gos_rt_trace_tracer_start_span" => Some("gos_rt_trace_tracer_start_span"),
        "gos_rt_trace_span_set_attribute" => Some("gos_rt_trace_span_set_attribute"),
        "gos_rt_trace_span_set_status" => Some("gos_rt_trace_span_set_status"),
        "gos_rt_trace_span_end" => Some("gos_rt_trace_span_end"),
        "gos_rt_trace_ended_to_otlp_json" => Some("gos_rt_trace_ended_to_otlp_json"),
        "gos_rt_bheap_len" => Some("gos_rt_bheap_len"),
        "gos_rt_bheap_is_empty" => Some("gos_rt_bheap_is_empty"),
        "gos_rt_bheap_clear" => Some("gos_rt_bheap_clear"),
        "gos_rt_bheap_max_new_i64" => Some("gos_rt_bheap_max_new_i64"),
        "gos_rt_bheap_max_from_vec_i64" => Some("gos_rt_bheap_max_from_vec_i64"),
        "gos_rt_bheap_max_from_vec_f64" => Some("gos_rt_bheap_max_from_vec_f64"),
        "gos_rt_bheap_max_push_i64" => Some("gos_rt_bheap_max_push_i64"),
        "gos_rt_bheap_max_push_f64" => Some("gos_rt_bheap_max_push_f64"),
        "gos_rt_bheap_max_pop_i64" => Some("gos_rt_bheap_max_pop_i64"),
        "gos_rt_bheap_max_pop_f64" => Some("gos_rt_bheap_max_pop_f64"),
        "gos_rt_bheap_max_peek_i64" => Some("gos_rt_bheap_max_peek_i64"),
        "gos_rt_bheap_min_new_i64" => Some("gos_rt_bheap_min_new_i64"),
        "gos_rt_bheap_min_from_vec_i64" => Some("gos_rt_bheap_min_from_vec_i64"),
        "gos_rt_bheap_min_from_vec_f64" => Some("gos_rt_bheap_min_from_vec_f64"),
        "gos_rt_bheap_min_push_i64" => Some("gos_rt_bheap_min_push_i64"),
        "gos_rt_bheap_min_push_f64" => Some("gos_rt_bheap_min_push_f64"),
        "gos_rt_bheap_min_pop_i64" => Some("gos_rt_bheap_min_pop_i64"),
        "gos_rt_bheap_min_pop_f64" => Some("gos_rt_bheap_min_pop_f64"),
        "gos_rt_bheap_min_peek_i64" => Some("gos_rt_bheap_min_peek_i64"),
        "gos_rt_vec_mark_shared" => Some("gos_rt_vec_mark_shared"),
        "gos_rt_url_query_escape" => Some("gos_rt_url_query_escape"),
        "gos_rt_url_path_escape" => Some("gos_rt_url_path_escape"),
        "gos_rt_url_query_unescape" => Some("gos_rt_url_query_unescape"),
        "gos_rt_url_path_unescape" => Some("gos_rt_url_path_unescape"),
        "gos_rt_bufio_scanner_new" => Some("gos_rt_bufio_scanner_new"),
        "gos_rt_bufio_scanner_scan" => Some("gos_rt_bufio_scanner_scan"),
        "gos_rt_bufio_scanner_text" => Some("gos_rt_bufio_scanner_text"),
        "gos_rt_http_client_new" => Some("gos_rt_http_client_new"),
        "gos_rt_http_client_get" => Some("gos_rt_http_client_get"),
        "gos_rt_http_client_post" => Some("gos_rt_http_client_post"),
        "gos_rt_http_client_put" => Some("gos_rt_http_client_put"),
        "gos_rt_http_client_options" => Some("gos_rt_http_client_options"),
        "gos_rt_http_client_delete" => Some("gos_rt_http_client_delete"),
        "gos_rt_http_client_head" => Some("gos_rt_http_client_head"),
        "gos_rt_http_client_builder_new" => Some("gos_rt_http_client_builder_new"),
        "gos_rt_http_client_builder_max_redirects" => {
            Some("gos_rt_http_client_builder_max_redirects")
        }
        "gos_rt_http_client_builder_timeout_ms" => Some("gos_rt_http_client_builder_timeout_ms"),
        "gos_rt_http_client_builder_cookie_jar" => Some("gos_rt_http_client_builder_cookie_jar"),
        "gos_rt_http_client_builder_proxy" => Some("gos_rt_http_client_builder_proxy"),
        "gos_rt_http_client_builder_build" => Some("gos_rt_http_client_builder_build"),
        "gos_rt_http_client_request" => Some("gos_rt_http_client_request"),
        "gos_rt_http_client_request_bytes" => Some("gos_rt_http_client_request_bytes"),
        "gos_rt_http_request_header" => Some("gos_rt_http_request_header"),
        "gos_rt_http_request_body" => Some("gos_rt_http_request_body"),
        "gos_rt_http_request_send" => Some("gos_rt_http_request_send"),
        "gos_rt_http_response_status" => Some("gos_rt_http_response_status"),
        "gos_rt_http_response_body" => Some("gos_rt_http_response_body"),
        "gos_rt_http_response_raw_bytes" => Some("gos_rt_http_response_raw_bytes"),
        "gos_rt_http_response_headers" => Some("gos_rt_http_response_headers"),
        "gos_rt_http_response_content_type" => Some("gos_rt_http_response_content_type"),
        "gos_rt_http_response_location" => Some("gos_rt_http_response_location"),
        "gos_rt_strconv_parse_f64_bytes" => Some("gos_rt_strconv_parse_f64_bytes"),
        "gos_rt_strconv_parse_i64_bytes" => Some("gos_rt_strconv_parse_i64_bytes"),
        "gos_rt_vec_get_i64" => Some("gos_rt_vec_get_i64"),
        // The JIT is not the perf target: resolve the bounds-free counted-loop
        // reader to the checked symbol so it keeps the lenient null/OOB -> 0
        // behavior, bit-identical to the VM and the checked path.
        "gos_rt_vec_get_i64_unchecked" => Some("gos_rt_vec_get_i64"),
        "gos_rt_vec_set_i64" => Some("gos_rt_vec_set_i64"),
        "gos_rt_vec_set_slots" => Some("gos_rt_vec_set_slots"),
        "gos_rt_vec_set_i128" => Some("gos_rt_vec_set_i128"),
        // The JIT is not the perf target: resolve the bounds-free counted-loop
        // writer to the checked symbol so it keeps the lenient null/OOB no-op
        // behavior, bit-identical to the VM and the checked path.
        "gos_rt_vec_set_i64_unchecked" => Some("gos_rt_vec_set_i64"),
        "gos_rt_vec_reserve_at_least" => Some("gos_rt_vec_reserve_at_least"),
        "gos_rt_vec_reserve_exact" => Some("gos_rt_vec_reserve_exact"),
        "gos_rt_vec_format_i64" => Some("gos_rt_vec_format_i64"),
        "gos_rt_vec_format_u64" => Some("gos_rt_vec_format_u64"),
        "gos_rt_tuple_format" => Some("gos_rt_tuple_format"),
        "gos_rt_tuple_format_desc" => Some("gos_rt_tuple_format_desc"),
        "gos_rt_map_format" => Some("gos_rt_map_format"),
        "gos_rt_map_format_tagged" => Some("gos_rt_map_format_tagged"),
        "gos_rt_set_format_i64" => Some("gos_rt_set_format_i64"),
        "gos_rt_set_format_string" => Some("gos_rt_set_format_string"),
        "gos_rt_set_format_u64" => Some("gos_rt_set_format_u64"),
        "gos_rt_deque_format" => Some("gos_rt_deque_format"),
        "gos_rt_queue_format" => Some("gos_rt_queue_format"),
        "gos_rt_stack_format" => Some("gos_rt_stack_format"),
        "gos_rt_bheap_max_format" => Some("gos_rt_bheap_max_format"),
        "gos_rt_bheap_min_format" => Some("gos_rt_bheap_min_format"),
        "gos_rt_chan_recv_option" => Some("gos_rt_chan_recv_option"),
        "gos_rt_chan_try_recv_option" => Some("gos_rt_chan_try_recv_option"),
        "gos_rt_result_new" => Some("gos_rt_result_new"),
        "gos_rt_result_new_f64" => Some("gos_rt_result_new_f64"),
        "gos_rt_result_disc" => Some("gos_rt_result_disc"),
        "gos_rt_result_payload" => Some("gos_rt_result_payload"),
        // Payload extract of a `w.upgrade()` result pinned in the shadow
        // local: identical bit-op to `gos_rt_result_payload` (the None
        // payload word is 0, so the extract is already null for None);
        // the distinct MIR name only marks the destination as owned.
        "gos_rt_weak_opt_payload" => Some("gos_rt_result_payload"),
        "gos_rt_result_payload_f64" => Some("gos_rt_result_payload_f64"),
        "gos_rt_result_payload_i128" => Some("gos_rt_result_payload_i128"),
        "gos_rt_option_unwrap" => Some("gos_rt_option_unwrap"),
        "gos_rt_result_unwrap" => Some("gos_rt_result_unwrap"),
        "gos_rt_result_unwrap_or" => Some("gos_rt_result_unwrap_or"),
        "gos_rt_result_ok" => Some("gos_rt_result_ok"),
        "gos_rt_result_err" => Some("gos_rt_result_err"),
        "gos_rt_result_ok_or" => Some("gos_rt_result_ok_or"),
        "gos_rt_result_ok_or_else" => Some("gos_rt_result_ok_or_else"),
        "gos_rt_result_is_ok" => Some("gos_rt_result_is_ok"),
        "gos_rt_result_is_err" => Some("gos_rt_result_is_err"),
        "gos_rt_select_new" => Some("gos_rt_select_new"),
        "gos_rt_select_arm_recv" => Some("gos_rt_select_arm_recv"),
        "gos_rt_select_arm_send" => Some("gos_rt_select_arm_send"),
        "gos_rt_select_arm_default" => Some("gos_rt_select_arm_default"),
        "gos_rt_select_wait" => Some("gos_rt_select_wait"),
        "gos_rt_select_value" => Some("gos_rt_select_value"),
        "gos_rt_select_free" => Some("gos_rt_select_free"),
        "gos_rt_set_new" => Some("gos_rt_set_new"),
        "gos_rt_btree_set_new" => Some("gos_rt_btree_set_new"),
        "gos_rt_set_insert" => Some("gos_rt_set_insert"),
        "gos_rt_set_insert_skey" => Some("gos_rt_set_insert_skey"),
        "gos_rt_set_contains" => Some("gos_rt_set_contains"),
        "gos_rt_set_contains_skey" => Some("gos_rt_set_contains_skey"),
        "gos_rt_set_remove" => Some("gos_rt_set_remove"),
        "gos_rt_set_remove_skey" => Some("gos_rt_set_remove_skey"),
        "gos_rt_set_len" => Some("gos_rt_set_len"),
        "gos_rt_set_to_vec_skey" => Some("gos_rt_set_to_vec_skey"),
        "gos_rt_set_union" => Some("gos_rt_set_union"),
        "gos_rt_set_intersection" => Some("gos_rt_set_intersection"),
        "gos_rt_set_intersection_skey" => Some("gos_rt_set_intersection_skey"),
        "gos_rt_set_intersection_to_vec" => Some("gos_rt_set_intersection_to_vec"),
        "gos_rt_set_intersection_to_vec_i64" => Some("gos_rt_set_intersection_to_vec_i64"),
        "gos_rt_set_intersection_to_vec_skey" => Some("gos_rt_set_intersection_to_vec_skey"),
        "gos_rt_set_difference" => Some("gos_rt_set_difference"),
        "gos_rt_set_symmetric_difference" => Some("gos_rt_set_symmetric_difference"),
        "gos_rt_set_is_subset" => Some("gos_rt_set_is_subset"),
        "gos_rt_set_is_superset" => Some("gos_rt_set_is_superset"),
        "gos_rt_set_is_disjoint" => Some("gos_rt_set_is_disjoint"),
        "gos_rt_str_as_bytes" => Some("gos_rt_str_as_bytes"),
        "gos_rt_string_from_utf8" => Some("gos_rt_string_from_utf8"),
        "gos_rt_vec_clone" => Some("gos_rt_vec_clone"),
        "gos_rt_map_inc_str_i64" => Some("gos_rt_map_inc_str_i64"),
        "gos_rt_map_inc_typed_str_i64" => Some("gos_rt_map_inc_typed_str_i64"),
        "gos_rt_map_or_insert_str_i64" => Some("gos_rt_map_or_insert_str_i64"),
        "gos_rt_map_or_insert_typed_str_i64" => Some("gos_rt_map_or_insert_typed_str_i64"),
        "gos_rt_map_get_or_typed_str_i64" => Some("gos_rt_map_get_or_typed_str_i64"),
        "gos_rt_map_get_typed_str_i64" => Some("gos_rt_map_get_typed_str_i64"),
        "gos_rt_map_contains_key_typed_str" => Some("gos_rt_map_contains_key_typed_str"),
        "gos_rt_map_remove_typed_str" => Some("gos_rt_map_remove_typed_str"),
        "gos_rt_map_or_insert_i64_i64" => Some("gos_rt_map_or_insert_i64_i64"),
        "gos_rt_errors_join_vec" => Some("gos_rt_errors_join_vec"),
        "gos_rt_errors_join" => Some("gos_rt_errors_join"),
        "gos_rt_json_value_object_n" => Some("gos_rt_json_value_object_n"),
        "gos_rt_http_response_set_header" => Some("gos_rt_http_response_set_header"),
        "gos_rt_http_response_set_content_type" => Some("gos_rt_http_response_set_content_type"),
        "gos_rt_http_response_set_body_bytes" => Some("gos_rt_http_response_set_body_bytes"),
        "gos_rt_http_response_with_header" => Some("gos_rt_http_response_with_header"),
        "gos_rt_http_response_get_header" => Some("gos_rt_http_response_get_header"),
        "gos_rt_http_request_set_header" => Some("gos_rt_http_request_set_header"),
        "gos_rt_http_request_get_header" => Some("gos_rt_http_request_get_header"),
        "gos_rt_gzip_encode" => Some("gos_rt_gzip_encode"),
        "gos_rt_gzip_decode" => Some("gos_rt_gzip_decode"),
        "gos_rt_sha256_hex" => Some("gos_rt_sha256_hex"),
        "gos_rt_sha512_hex" => Some("gos_rt_sha512_hex"),
        "gos_rt_blake3_hex" => Some("gos_rt_blake3_hex"),
        "gos_rt_hmac_sha256_hex" => Some("gos_rt_hmac_sha256_hex"),
        "gos_rt_chunked_encode" => Some("gos_rt_chunked_encode"),
        "gos_rt_chunked_decode" => Some("gos_rt_chunked_decode"),
        "gos_rt_sse_encode_event" => Some("gos_rt_sse_encode_event"),
        "gos_rt_sse_encode_comment" => Some("gos_rt_sse_encode_comment"),
        "gos_rt_sse_encode_retry" => Some("gos_rt_sse_encode_retry"),
        "gos_rt_mw_new_request_id" => Some("gos_rt_mw_new_request_id"),
        "gos_rt_mw_accepts_gzip" => Some("gos_rt_mw_accepts_gzip"),
        "gos_rt_mw_decode_basic_auth" => Some("gos_rt_mw_decode_basic_auth"),
        "gos_rt_ws_accept" => Some("gos_rt_ws_accept"),
        "gos_rt_ws_is_upgrade" => Some("gos_rt_ws_is_upgrade"),
        "gos_rt_ws_accept_key" => Some("gos_rt_ws_accept_key"),
        "gos_rt_static_mime_for_path" => Some("gos_rt_static_mime_for_path"),
        "gos_rt_static_serve_file" => Some("gos_rt_static_serve_file"),
        "gos_rt_router_new" => Some("gos_rt_router_new"),
        "gos_rt_router_add" => Some("gos_rt_router_add"),
        "gos_rt_router_get" => Some("gos_rt_router_get"),
        "gos_rt_router_post" => Some("gos_rt_router_post"),
        "gos_rt_router_put" => Some("gos_rt_router_put"),
        "gos_rt_router_delete" => Some("gos_rt_router_delete"),
        "gos_rt_router_patch" => Some("gos_rt_router_patch"),
        "gos_rt_router_head" => Some("gos_rt_router_head"),
        "gos_rt_router_options" => Some("gos_rt_router_options"),
        "gos_rt_router_add_fn" => Some("gos_rt_router_add_fn"),
        "gos_rt_router_add_pattern" => Some("gos_rt_router_add_pattern"),
        "gos_rt_router_lookup" => Some("gos_rt_router_lookup"),
        "gos_rt_router_get_fn" => Some("gos_rt_router_get_fn"),
        "gos_rt_router_post_fn" => Some("gos_rt_router_post_fn"),
        "gos_rt_router_put_fn" => Some("gos_rt_router_put_fn"),
        "gos_rt_router_delete_fn" => Some("gos_rt_router_delete_fn"),
        "gos_rt_router_patch_fn" => Some("gos_rt_router_patch_fn"),
        "gos_rt_router_head_fn" => Some("gos_rt_router_head_fn"),
        "gos_rt_router_options_fn" => Some("gos_rt_router_options_fn"),
        "gos_rt_router_serve" => Some("gos_rt_router_serve"),
        "gos_rt_file_server_new" => Some("gos_rt_file_server_new"),
        "gos_rt_file_server_serve" => Some("gos_rt_file_server_serve"),
        "gos_rt_native_client_new" => Some("gos_rt_native_client_new"),
        "gos_rt_native_client_get" => Some("gos_rt_native_client_get"),
        "gos_rt_nc_get" => Some("gos_rt_nc_get"),
        "gos_rt_nc_delete" => Some("gos_rt_nc_delete"),
        "gos_rt_nc_post" => Some("gos_rt_nc_post"),
        "gos_rt_nc_put" => Some("gos_rt_nc_put"),
        "gos_rt_proxy_new" => Some("gos_rt_proxy_new"),
        "gos_rt_proxy_forward" => Some("gos_rt_proxy_forward"),
        "gos_rt_proxy_forward_url" => Some("gos_rt_proxy_forward_url"),
        "gos_rt_ws_frame_text" => Some("gos_rt_ws_frame_text"),
        "gos_rt_slog_info" => Some("gos_rt_slog_info"),
        "gos_rt_slog_warn" => Some("gos_rt_slog_warn"),
        "gos_rt_slog_error" => Some("gos_rt_slog_error"),
        "gos_rt_slog_debug" => Some("gos_rt_slog_debug"),
        "gos_rt_testing_check" => Some("gos_rt_testing_check"),
        "gos_rt_testing_check_eq_i64" => Some("gos_rt_testing_check_eq_i64"),
        "gos_rt_testing_wait_for_scheduler_idle" => Some("gos_rt_testing_wait_for_scheduler_idle"),
        "gos_rt_httptest_server" => Some("gos_rt_httptest_server"),
        "gos_rt_image_new" => Some("gos_rt_image_new"),
        "gos_rt_image_filled" => Some("gos_rt_image_filled"),
        "gos_rt_image_decode_base64" => Some("gos_rt_image_decode_base64"),
        "gos_rt_image_width" => Some("gos_rt_image_width"),
        "gos_rt_image_height" => Some("gos_rt_image_height"),
        "gos_rt_image_pixel" => Some("gos_rt_image_pixel"),
        "gos_rt_image_set_pixel" => Some("gos_rt_image_set_pixel"),
        "gos_rt_image_encode_png_base64" => Some("gos_rt_image_encode_png_base64"),
        "gos_rt_image_encode_jpeg_base64" => Some("gos_rt_image_encode_jpeg_base64"),
        "gos_rt_runtime_scheduler_stats_json" => Some("gos_rt_runtime_scheduler_stats_json"),
        "gos_rt_pprof_cpu_profile" => Some("gos_rt_pprof_cpu_profile"),
        "gos_rt_pprof_heap_profile" => Some("gos_rt_pprof_heap_profile"),
        "gos_rt_pprof_goroutine_profile" => Some("gos_rt_pprof_goroutine_profile"),
        "gos_rt_pprof_mutex_profile" => Some("gos_rt_pprof_mutex_profile"),
        "gos_rt_pprof_block_profile" => Some("gos_rt_pprof_block_profile"),
        "gos_rt_pprof_execution_trace" => Some("gos_rt_pprof_execution_trace"),
        "gos_rt_pprof_route" => Some("gos_rt_pprof_route"),
        "gos_rt_runtime_cycle_collection_supported" => {
            Some("gos_rt_runtime_cycle_collection_supported")
        }
        "gos_rt_parse_i64_result" => Some("gos_rt_parse_i64_result"),
        "gos_rt_iter_count_by_i64" => Some("gos_rt_iter_count_by_i64"),
        "gos_rt_lazy_iter_range_i64" => Some("gos_rt_lazy_iter_range_i64"),
        "gos_rt_lazy_iter_range_from_i64" => Some("gos_rt_lazy_iter_range_from_i64"),
        "gos_rt_lazy_iter_range_inclusive_i64" => Some("gos_rt_lazy_iter_range_inclusive_i64"),
        "gos_rt_lazy_iter_from_vec_i64" => Some("gos_rt_lazy_iter_from_vec_i64"),
        "gos_rt_lazy_iter_str_chars" => Some("gos_rt_lazy_iter_str_chars"),
        "gos_rt_lazy_iter_str_bytes" => Some("gos_rt_lazy_iter_str_bytes"),
        "gos_rt_lazy_iter_repeat_i64" => Some("gos_rt_lazy_iter_repeat_i64"),
        "gos_rt_lazy_iter_once_i64" => Some("gos_rt_lazy_iter_once_i64"),
        "gos_rt_lazy_iter_take_i64" => Some("gos_rt_lazy_iter_take_i64"),
        "gos_rt_lazy_iter_skip_i64" => Some("gos_rt_lazy_iter_skip_i64"),
        "gos_rt_lazy_iter_chain_i64" => Some("gos_rt_lazy_iter_chain_i64"),
        "gos_rt_lazy_iter_enumerate_i64" => Some("gos_rt_lazy_iter_enumerate_i64"),
        "gos_rt_lazy_iter_zip_i64" => Some("gos_rt_lazy_iter_zip_i64"),
        "gos_rt_lazy_iter_map_i64" => Some("gos_rt_lazy_iter_map_i64"),
        "gos_rt_lazy_iter_filter_i64" => Some("gos_rt_lazy_iter_filter_i64"),
        "gos_rt_lazy_iter_collect_i64" => Some("gos_rt_lazy_iter_collect_i64"),
        "gos_rt_lazy_iter_collect_aggr" => Some("gos_rt_lazy_iter_collect_aggr"),
        "gos_rt_lazy_iter_from_vec_aggr" => Some("gos_rt_lazy_iter_from_vec_aggr"),
        "gos_rt_lazy_iter_collect_pair_i64" => Some("gos_rt_lazy_iter_collect_pair_i64"),
        "gos_rt_lazy_iter_count_i64" => Some("gos_rt_lazy_iter_count_i64"),
        "gos_rt_lazy_iter_count_pair_i64" => Some("gos_rt_lazy_iter_count_pair_i64"),
        "gos_rt_lazy_iter_drop_i64" => Some("gos_rt_lazy_iter_drop_i64"),
        "gos_rt_lazy_iter_drop_pair_i64" => Some("gos_rt_lazy_iter_drop_pair_i64"),
        "gos_rt_lazy_iter_sum_i64" => Some("gos_rt_lazy_iter_sum_i64"),
        "gos_rt_lazy_iter_product_i64" => Some("gos_rt_lazy_iter_product_i64"),
        "gos_rt_lazy_iter_min_i64" => Some("gos_rt_lazy_iter_min_i64"),
        "gos_rt_lazy_iter_max_i64" => Some("gos_rt_lazy_iter_max_i64"),
        "gos_rt_lazy_iter_next_i64" => Some("gos_rt_lazy_iter_next_i64"),
        "gos_rt_lazy_iter_fold_i64" => Some("gos_rt_lazy_iter_fold_i64"),
        "gos_rt_lazy_iter_any_i64" => Some("gos_rt_lazy_iter_any_i64"),
        "gos_rt_lazy_iter_all_i64" => Some("gos_rt_lazy_iter_all_i64"),
        "gos_rt_iter_all_ptr" => Some("gos_rt_iter_all_ptr"),
        "gos_rt_iter_any_ptr" => Some("gos_rt_iter_any_ptr"),
        "gos_rt_iter_any_f64" => Some("gos_rt_iter_any_f64"),
        "gos_rt_lazy_iter_find_i64" => Some("gos_rt_lazy_iter_find_i64"),
        "gos_rt_iter_filter_map_i64" => Some("gos_rt_iter_filter_map_i64"),
        "gos_rt_iter_find_map_i64" => Some("gos_rt_iter_find_map_i64"),
        "gos_rt_iter_flat_map_i64" => Some("gos_rt_iter_flat_map_i64"),
        "gos_rt_iter_flat_map_arr_i64" => Some("gos_rt_iter_flat_map_arr_i64"),
        "gos_rt_iter_group_by_i64" => Some("gos_rt_iter_group_by_i64"),
        "gos_rt_iter_max_by_i64" => Some("gos_rt_iter_max_by_i64"),
        "gos_rt_iter_max_by_key_f64" => Some("gos_rt_iter_max_by_key_f64"),
        "gos_rt_iter_max_by_key_i64" => Some("gos_rt_iter_max_by_key_i64"),
        "gos_rt_iter_max_by_key_ptr" => Some("gos_rt_iter_max_by_key_ptr"),
        "gos_rt_iter_min_by_i64" => Some("gos_rt_iter_min_by_i64"),
        "gos_rt_iter_min_by_key_f64" => Some("gos_rt_iter_min_by_key_f64"),
        "gos_rt_iter_min_by_key_i64" => Some("gos_rt_iter_min_by_key_i64"),
        "gos_rt_iter_min_by_key_ptr" => Some("gos_rt_iter_min_by_key_ptr"),
        "gos_rt_iter_partition_i64" => Some("gos_rt_iter_partition_i64"),
        "gos_rt_iter_chunk_by_size_i64" => Some("gos_rt_iter_chunk_by_size_i64"),
        "gos_rt_iter_dedup_i64" => Some("gos_rt_iter_dedup_i64"),
        "gos_rt_iter_enumerate_i64" => Some("gos_rt_iter_enumerate_i64"),
        "gos_rt_iter_flatten_i64" => Some("gos_rt_iter_flatten_i64"),
        "gos_rt_iter_for_each_ptr" => Some("gos_rt_iter_for_each_ptr"),
        "gos_rt_iter_pairwise_i64" => Some("gos_rt_iter_pairwise_i64"),
        "gos_rt_iter_unzip_i64" => Some("gos_rt_iter_unzip_i64"),
        "gos_rt_iter_windowed_i64" => Some("gos_rt_iter_windowed_i64"),
        "gos_rt_iter_zip_i64" => Some("gos_rt_iter_zip_i64"),
        "gos_rt_println_fn_f64" => Some("gos_rt_println_fn_f64"),
        "gos_rt_println_fn_i64" => Some("gos_rt_println_fn_i64"),
        "gos_rt_println_fn_str_word" => Some("gos_rt_println_fn_str_word"),
        "gos_rt_iter_position_i64" => Some("gos_rt_iter_position_i64"),
        "gos_rt_iter_position_ptr" => Some("gos_rt_iter_position_ptr"),
        "gos_rt_iter_product_by_i64" => Some("gos_rt_iter_product_by_i64"),
        "gos_rt_iter_reduce_i64" => Some("gos_rt_iter_reduce_i64"),
        "gos_rt_iter_scan_i64" => Some("gos_rt_iter_scan_i64"),
        "gos_rt_iter_skip_while_i64" => Some("gos_rt_iter_skip_while_i64"),
        "gos_rt_iter_sorted_by_i64" => Some("gos_rt_iter_sorted_by_i64"),
        "gos_rt_iter_sorted_by_key_f64" => Some("gos_rt_iter_sorted_by_key_f64"),
        "gos_rt_iter_sorted_by_key_i64" => Some("gos_rt_iter_sorted_by_key_i64"),
        "gos_rt_iter_take_while_i64" => Some("gos_rt_iter_take_while_i64"),
        "gos_rt_option_and_then" => Some("gos_rt_option_and_then"),
        "gos_rt_option_default_with" => Some("gos_rt_option_default_with"),
        "gos_rt_option_filter" => Some("gos_rt_option_filter"),
        "gos_rt_option_flatten" => Some("gos_rt_option_flatten"),
        "gos_rt_option_iter" => Some("gos_rt_option_iter"),
        "gos_rt_option_or" => Some("gos_rt_option_or"),
        "gos_rt_option_or_else" => Some("gos_rt_option_or_else"),
        "gos_rt_option_zip" => Some("gos_rt_option_zip"),
        "gos_rt_result_and_then" => Some("gos_rt_result_and_then"),
        "gos_rt_result_or_else" => Some("gos_rt_result_or_else"),
        "gos_rt_result_to_opt_err" => Some("gos_rt_result_to_opt_err"),
        "gos_rt_result_to_opt_ok" => Some("gos_rt_result_to_opt_ok"),
        "gos_rt_result_map_err" => Some("gos_rt_result_map_err"),
        "gos_rt_result_map" => Some("gos_rt_result_map"),
        "gos_rt_flag_cell_load_str" => Some("gos_rt_flag_cell_load_str"),
        "gos_rt_flag_cell_load_i64" => Some("gos_rt_flag_cell_load_i64"),
        "gos_rt_flag_cell_load_bool" => Some("gos_rt_flag_cell_load_bool"),
        "gos_rt_flag_cell_load_f64" => Some("gos_rt_flag_cell_load_f64"),
        "gos_rt_flag_cell_load_vec" => Some("gos_rt_flag_cell_load_vec"),
        // Plain ascending sort for Vec<i64>.
        "gos_rt_arr_reverse" => Some("gos_rt_arr_reverse"),
        "gos_rt_arr_sort_i64" => Some("gos_rt_arr_sort_i64"),
        "gos_rt_arr_sort_str" => Some("gos_rt_arr_sort_str"),
        "gos_rt_arr_sort_tuple" => Some("gos_rt_arr_sort_tuple"),
        "gos_rt_vec_reverse" => Some("gos_rt_vec_reverse"),
        "gos_rt_vec_sort_i64" => Some("gos_rt_vec_sort_i64"),
        "gos_rt_vec_sort_str" => Some("gos_rt_vec_sort_str"),
        "gos_rt_vec_sort_tuple" => Some("gos_rt_vec_sort_tuple"),
        // Sort-by callbacks for fixed-array / Vec receivers.
        "gos_rt_arr_sort_by_f64" => Some("gos_rt_arr_sort_by_f64"),
        "gos_rt_arr_sort_by_i64" => Some("gos_rt_arr_sort_by_i64"),
        "gos_rt_vec_sort_by_f64" => Some("gos_rt_vec_sort_by_f64"),
        "gos_rt_vec_sort_by_i64" => Some("gos_rt_vec_sort_by_i64"),
        // Stride-aware sort_by for multi-slot aggregate elements
        // (Tuple / struct). The comparator receives element
        // pointers; the cranelift ABI passes aggregates that way
        // already so the user closure body works unchanged.
        "gos_rt_arr_sort_by_aggr" => Some("gos_rt_arr_sort_by_aggr"),
        "gos_rt_vec_sort_by_aggr" => Some("gos_rt_vec_sort_by_aggr"),
        "gos_rt_json_free" => Some("gos_rt_json_free"),
        "gos_rt_json_set" => Some("gos_rt_json_set"),
        "gos_rt_arr_iter" => Some("gos_rt_arr_iter"),
        "gos_rt_arr_iter_next" => Some("gos_rt_arr_iter_next"),
        // The registry owns the canonical `&'static str` for every
        // runtime symbol, so a helper the arms above do not spell still
        // interns here rather than dropping out of native lowering.
        _ => gossamer_abi::lookup(name).map(|entry| entry.name),
    }
}
