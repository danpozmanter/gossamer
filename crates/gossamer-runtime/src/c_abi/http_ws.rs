...
/// connected handle to the user handler on a dedicated per-connection
/// thread (Go model). Reuses `accept_serve` from the HTTP server so the
/// connection-cap / shutdown behaviour matches `http::serve`.
#[unsafe(no_mangle)]
