...
pub(crate) fn install_http_native_client(globals: &mut Vec<(&'static str, Value)>) {
    for (name, call) in [
...
pub(crate) fn builtin_native_client_get(args: &[Value]) -> RuntimeResult<Value> {
...
pub(crate) fn builtin_native_client_post(args: &[Value]) -> RuntimeResult<Value> {
    let url = arg_str(args.first());
...
pub(crate) fn builtin_native_client_put(args: &[Value]) -> RuntimeResult<Value> {
    let url = arg_str(args.first());
    let body = bytes_from_value(args.get(1).unwrap_or(&Value::Unit));
    let content_type = arg_str(args.get(2));
    let ct = if content_type.is_empty() {
...
pub(crate) fn builtin_native_client_delete(args: &[Value]) -> RuntimeResult<Value> {
    let url = arg_str(args.first());
    let client = gossamer_std::http_native_client::NativeClient::new();
