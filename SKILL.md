...
detached form. `spawn(f, reason: "fetch users")` labels the goroutine, and
the cohort's reports name a labelled child by that label instead of by its
spawn index. Typed channels: `recv()` blocks until a value arrives or the
