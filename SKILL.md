...
- **A collection traverses eagerly; `iter()` makes it lazy.** `xs.map(f)`
  on a `Vec`, array, slice, or `Set` walks the values it already holds and
...
- **A callback has two shorthands.** A std free function named in
  ...
  and `xs.map(base64::encode)` need no `|v|` (a variadic like
  `fmt::format` has no fixed parameter list and reports GT0015 - write
  that closure). A `$`-headed projection in an argument is the closure
...
lists commands. `%info`/`%i` searches public symbols and the session, showing
item help plus a type's fields, implemented traits, and methods (and a trait's
implementors); `%explain`/`%e` answers the same for a binding in receiver form;
