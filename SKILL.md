...
- **`to_string()` renders whatever `{}` renders.** A scalar, a tuple, a
  ...
  is the same override for `{:?}`. A trait names behaviour, never a value's
  type: `fn f(x: Display)` reports GT0071, and an `impl` header naming a trait
  nothing declares reports GT0070.
